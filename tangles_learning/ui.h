...
struct ui_callbacks {
    ...
    function<void (ui_window*,const vec2f&)>        mouse_pos;
    function<void (ui_window*,const vec2f&)>        scroll;
...
inline ui_window* ui_init_window(const vec2i& size, const string& title, int samples = 1) {
    // init glfw
    auto ok = glfwInit();
...
// windows properies
inline vec2i ui_window_size(ui_window* window) { auto size = zero2i; glfwGetWindowSize(window, &size.x, &size.y); return size; }
// windows properies
inline vec2i ui_framebuffer_size(ui_window* window) { auto size = zero2i; glfwGetFramebufferSize(window, &size.x, &size.y); return size; }

// mouse button
inline bool ui_mouse_button(ui_window* window, int button) { return glfwGetMouseButton(window, button); }
inline vec2f ui_mouse_pos(ui_window* window) { double x, y; glfwGetCursorPos(window, &x, &y); return vec2f(x,y); }
...
// image saving
inline image3f ui_capture_image(ui_window* window) {
    // create an image
    auto image = image3f(ui_framebuffer_size(window));
    // grab screen pixels
    glReadPixels(0, 0, image.width(), image.height(), GL_RGB, GL_FLOAT, &image.at(0,0));
    // done
    return image;
}
...
inline bool ui_slider_widget(ui_window* window, const string& label, float* value, const range1f& range = {0,1}) {
    return ImGui::SliderFloat(label.c_str(), value, range.min, range.max);
}
inline bool ui_slider_widget(ui_window* window, const string& label, vec2f* value, const range1f& range = {0,1}) {
    return ImGui::SliderFloat2(label.c_str(), value->data(), range.min, range.max);
}
inline bool ui_slider_widget(ui_window* window, const string& label, vec3f* value, const range1f& range = {0,1}) {
    return ImGui::SliderFloat3(label.c_str(), value->data(), range.min, range.max);
}
inline bool ui_color_widget(ui_window* window, const string& label, vec3f* value) {
    return ImGui::ColorEdit3(label.c_str(), value->data());
}
inline bool ui_colorf_widget(ui_window* window, const string& label, vec3f* value) {
    auto changed = false;
    changed = changed or ui_color_widget(window, label+"##color", value);
    changed = changed or ui_slider_widget(window, label+"##float", value);
    return changed;
}
inline bool ui_colorl_widget(ui_window* window, const string& label, vec3f* value, const range1f& range = {0,1}) {
    auto changed = false;
    changed = changed or ui_color_widget(window, label+"##color", value);
    auto l = mean(*value); auto black = l == 0;
    if(ui_slider_widget(window, label+"##float", &l, range)) {
        changed = true;
        if(black) *value = {l,l,l};
        else *value *= l / mean(*value);
    }
    return changed;
}
inline bool ui_colori_widget(ui_window* window, const string& label, vec3f* value, const range1f& range = {0,1000}) {
    auto m = max(value->x,max(value->y,value->z));
    auto black = m == 0;
    auto c = (black) ? zero3f : *value / m;
    auto changed = false;
...
inline bool ui_combo_widget(ui_window* window, const string& label, const vector<string>& labels, int* value) {
    auto str = ""s; for(auto s : labels) str += s + '\0'; str += '\0';
...
inline bool ui_rotation_widget(ui_window* window, const string& label, frame3f* value) {
    auto rot = euler_angles(frame_to_matrix(*value));
    auto edited = ui_slider_widget(window, label, &rot, {-pif,pif});
    if(edited) { auto r = euler_matrix(rot); auto ff = matrix_to_frame(r); ff.o = value->o; *value = ff; }
    return edited;
}
inline bool ui_frame_widget(ui_window* window, const string& label, frame3f* value, const range1f& range = {-10,10}) {
...
#endif
    if(callbacks->mouse_pos) callbacks->mouse_pos(window,vec2f(x,y));
}
...
#endif
    if(callbacks->scroll) callbacks->scroll(window,vec2f(x,y));
}
