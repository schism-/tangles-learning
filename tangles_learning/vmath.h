...
inline bool operator!=(const vec2f& a, const vec2f& b) { return not operator==(a,b); }

// 2d lexical ordering  ------------------------------
inline bool operator<(const vec2f& a, const vec2f& b) { return a.x<b.x or (a.x==b.x and a.y<b.y); }
...
struct vec2d {
    ...
    // Vector constructor
    template<typename T> vec2d(const vector<T>& v) { error_if_not(v.size() == size, "bad length"); std::copy(v.begin(),v.end(),begin()); }
...
// 2d component-wise equality  -----------------------
inline bool operator==(const vec2d& a, const vec2d& b) { return a.x==b.x and a.y==b.y; }
inline bool operator!=(const vec2d& a, const vec2d& b) { return not operator==(a,b); }

// 2d lexical ordering  ------------------------------
inline bool operator<(const vec2d& a, const vec2d& b) { return a.x<b.x or (a.x==b.x and a.y<b.y); }

// 2d component-wise arithmetic operators -----------
inline vec2d operator-(const vec2d& a) { return vec2d(-a.x, -a.y); }
inline vec2d operator+(const vec2d& a, const vec2d& b) { return vec2d(a.x+b.x, a.y+b.y); }
inline vec2d& operator+=(vec2d& a, const vec2d& b) { a.x+=b.x; a.y+=b.y; return a; }
inline vec2d operator-(const vec2d& a, const vec2d& b) { return vec2d(a.x-b.x, a.y-b.y); }
inline vec2d& operator-=(vec2d& a, const vec2d& b) { a.x-=b.x; a.y-=b.y; return a; }
inline vec2d operator*(const vec2d& a, const vec2d& b) { return vec2d(a.x*b.x, a.y*b.y); }
inline vec2d& operator*=(vec2d& a, const vec2d& b) { a.x*=b.x; a.y*=b.y; return a; }
inline vec2d operator/(const vec2d& a, const vec2d& b) { return vec2d(a.x/b.x, a.y/b.y); }
inline vec2d& operator/=(vec2d& a, const vec2d& b) { a.x/=b.x; a.y/=b.y; return a; }
inline vec2d operator*(const vec2d& a, double b) { return vec2d(a.x*b, a.y*b); }
inline vec2d operator*(double a, const vec2d& b) { return vec2d(a*b.x, a*b.y); }
inline vec2d& operator*=(vec2d& a, double b) { a.x*=b; a.y*=b; return a; }
template<typename T, typename R> inline vec2d operator*(const vec2d& a, const R& b) { return vec2d(a.x*b, a.y*b); }
template<typename T, typename R> inline vec2d operator*(const R& a, const vec2d& b) { return vec2d(a*b.x, a*b.y); }
template<typename T, typename R> inline vec2d& operator*=(vec2d& a, const R& b) { a.x*=b; a.y*=b; return a; }
inline vec2d operator/(const vec2d& a, double b) { return vec2d(a.x/b, a.y/b); }
inline vec2d operator/(double a, const vec2d& b) { return vec2d(a/b.x, a/b.y); }
inline vec2d& operator/=(vec2d& a, double b) { a.x/=b; a.y/=b; return a; }
template<typename T, typename R> inline vec2d operator/(const vec2d& a, const R& b) { return vec2d(a.x/b, a.y/b); }
...
// 2d component-wise functions ----------------------
// max, min, clamp
inline vec2d max(const vec2d& a, const vec2d& b) { return vec2d(max(a.x,b.x), max(a.y,b.y)); }
inline vec2d max(const vec2d& a, double b) { return vec2d(max(a.x,b), max(a.y,b)); }
inline vec2d max(double a, const vec2d& b) { return vec2d(max(a,b.x), max(a,b.y)); }
inline vec2d min(const vec2d& a, const vec2d& b) { return vec2d(min(a.x,b.x), min(a.y,b.y)); }
inline vec2d min(const vec2d& a, double b) { return vec2d(min(a.x,b), min(a.y,b)); }
inline vec2d min(double a, const vec2d& b) { return vec2d(min(a,b.x), min(a,b.y)); }
inline vec2d clamp(const vec2d& x, const vec2d& m, const vec2d& M) { return vec2d(clamp(x.x,m.x,M.x), clamp(x.y,m.y,M.y)); }
inline vec2d clamp(const vec2d& x, double m, double M) { return vec2d(clamp(x.x,m,M), clamp(x.y,m,M)); }
inline double mean(const vec2d& a) { return (a.x + a.y) / 2; }
...
inline double lengthSqr(const vec2d& a) { return dot(a,a); }
// normalize
inline vec2d normalize(const vec2d& a) { auto l = length(a); if (l==0) return vec2d(0,0); else return a * 1/l; }
...
inline bool operator!=(const vec3f& a, const vec3f& b) { return not operator==(a,b); }

// 3d lexical ordering  ------------------------------
inline bool operator<(const vec3f& a, const vec3f& b) { return a.x<b.x or (a.x==b.x and a.y<b.y) or (a.x==b.x and a.y==b.y and a.z<b.z); }
...
inline bool operator!=(const vec4f& a, const vec4f& b) { return not operator==(a,b); }

// 4d lexical ordering  ------------------------------
inline bool operator<(const vec4f& a, const vec4f& b) { return a.x<b.x or (a.x==b.x and a.y<b.y) or (a.x==b.x and a.y==b.y and a.z<b.z) or (a.x==b.x and a.y==b.y and a.z==b.z and a.w<b.w); }
...
struct vec4d {
    ...
    // Vector constructor
    template<typename T> vec4d(const vector<T>& v) { error_if_not(v.size() == size, "bad length"); std::copy(v.begin(),v.end(),begin()); }
...
// 4d component-wise equality  -----------------------
inline bool operator==(const vec4d& a, const vec4d& b) { return a.x==b.x and a.y==b.y and a.z==b.z and a.w==b.w; }
inline bool operator!=(const vec4d& a, const vec4d& b) { return not operator==(a,b); }

// 4d lexical ordering  ------------------------------
inline bool operator<(const vec4d& a, const vec4d& b) { return a.x<b.x or (a.x==b.x and a.y<b.y) or (a.x==b.x and a.y==b.y and a.z<b.z) or (a.x==b.x and a.y==b.y and a.z==b.z and a.w<b.w); }

// 4d component-wise arithmetic operators -----------
inline vec4d operator-(const vec4d& a) { return vec4d(-a.x, -a.y, -a.z, -a.w); }
inline vec4d operator+(const vec4d& a, const vec4d& b) { return vec4d(a.x+b.x, a.y+b.y, a.z+b.z, a.w+b.w); }
inline vec4d& operator+=(vec4d& a, const vec4d& b) { a.x+=b.x; a.y+=b.y; a.z+=b.z; a.w+=b.w; return a; }
inline vec4d operator-(const vec4d& a, const vec4d& b) { return vec4d(a.x-b.x, a.y-b.y, a.z-b.z, a.w-b.w); }
inline vec4d& operator-=(vec4d& a, const vec4d& b) { a.x-=b.x; a.y-=b.y; a.z-=b.z; a.w-=b.w; return a; }
inline vec4d operator*(const vec4d& a, const vec4d& b) { return vec4d(a.x*b.x, a.y*b.y, a.z*b.z, a.w*b.w); }
inline vec4d& operator*=(vec4d& a, const vec4d& b) { a.x*=b.x; a.y*=b.y; a.z*=b.z; a.w*=b.w; return a; }
inline vec4d operator/(const vec4d& a, const vec4d& b) { return vec4d(a.x/b.x, a.y/b.y, a.z/b.z, a.w/b.w); }
inline vec4d& operator/=(vec4d& a, const vec4d& b) { a.x/=b.x; a.y/=b.y; a.z/=b.z; a.w/=b.w; return a; }
inline vec4d operator*(const vec4d& a, double b) { return vec4d(a.x*b, a.y*b, a.z*b, a.w*b); }
inline vec4d operator*(double a, const vec4d& b) { return vec4d(a*b.x, a*b.y, a*b.z, a*b.w); }
inline vec4d& operator*=(vec4d& a, double b) { a.x*=b; a.y*=b; a.z*=b; a.w*=b; return a; }
template<typename T, typename R> inline vec4d operator*(const vec4d& a, const R& b) { return vec4d(a.x*b, a.y*b, a.z*b, a.w*b); }
template<typename T, typename R> inline vec4d operator*(const R& a, const vec4d& b) { return vec4d(a*b.x, a*b.y, a*b.z, a*b.w); }
template<typename T, typename R> inline vec4d& operator*=(vec4d& a, const R& b) { a.x*=b; a.y*=b; a.z*=b; a.w*=b; return a; }
inline vec4d operator/(const vec4d& a, double b) { return vec4d(a.x/b, a.y/b, a.z/b, a.w/b); }
inline vec4d operator/(double a, const vec4d& b) { return vec4d(a/b.x, a/b.y, a/b.z, a/b.w); }
inline vec4d& operator/=(vec4d& a, float b) { a.x/=b; a.y/=b; a.z/=b; a.w/=b; return a; }
template<typename T, typename R> inline vec4d operator/(const vec4d& a, const R& b) { return vec4d(a.x/b, a.y/b, a.z/b, a.w/b); }
template<typename T, typename R> inline vec4d operator/(const R& a, const vec4d& b) { return vec4d(a/b.x, a/b.y, a/b.z, a/b.w); }
template<typename T, typename R> inline vec4d& operator/=(vec4d& a, const R& b) { a.x/=b; a.y/=b; a.z/=b; a.w/=b; return a; }
...
// 4d component-wise functions ----------------------
// max, min, clamp
inline vec4d max(const vec4d& a, const vec4d& b) { return vec4d(max(a.x,b.x), max(a.y,b.y), max(a.z,b.z), max(a.w,b.w)); }
inline vec4d max(const vec4d& a, double b) { return vec4d(max(a.x,b), max(a.y,b), max(a.z,b), max(a.w,b)); }
inline vec4d max(double a, const vec4d& b) { return vec4d(max(a,b.x), max(a,b.y), max(a,b.z), max(a,b.w)); }
inline vec4d min(const vec4d& a, const vec4d& b) { return vec4d(min(a.x,b.x), min(a.y,b.y), min(a.z,b.z), min(a.w,b.w)); }
inline vec4d min(const vec4d& a, double b) { return vec4d(min(a.x,b), min(a.y,b), min(a.z,b), min(a.w,b)); }
inline vec4d min(double a, const vec4d& b) { return vec4d(min(a,b.x), min(a,b.y), min(a,b.z), min(a,b.w)); }
inline vec4d clamp(const vec4d& x, const vec4d& m, const vec4d& M) { return vec4d(clamp(x.x,m.x,M.x), clamp(x.y,m.y,M.y), clamp(x.z,m.z,M.z), clamp(x.w,m.w,M.w)); }
inline vec4d clamp(const vec4d& x, double m, double M) { return vec4d(clamp(x.x,m,M), clamp(x.y,m,M), clamp(x.z,m,M), clamp(x.w,m,M)); }
inline float mean(const vec4d& a) { return (a.x + a.y + a.z + a.w) / 4; }
inline vec4d pow(const vec4d& a, double b) { return vec4d(pow(a.x,b),pow(a.y,b),pow(a.z,b),pow(a.w,b)); }
inline vec4d sqrt(const vec4d& a) { return vec4d(sqrt(a.x),sqrt(a.y),sqrt(a.z),sqrt(a.w)); }
inline vec4d exp(const vec4d& a) { return vec4d(exp(a.x),exp(a.y),exp(a.z),exp(a.w)); }

// 4d vector operations -----------------------------
// dot product
inline double dot(const vec4d& a, const vec4d& b) { return a.x*b.x+a.y*b.y+a.z*b.z+a.w*b.w; }
// length and lengthSqr
inline double length(const vec4d& a) { return sqrt(dot(a,a)); }
inline double lengthSqr(const vec4d& a) { return dot(a,a); }
// normnalization
inline vec4d normalize(const vec4d& a) { auto l = length(a); if (l==0) return vec4d(0,0,0,0); else return a * 1/l; }
...
inline bool operator!=(const vec2i& a, const vec2i& b) { return not operator==(a,b); }

// 2d lexical ordering  ------------------------------
inline bool operator<(const vec2i& a, const vec2i& b) { return a.x<b.x or (a.x==b.x and a.y<b.y); }
...
inline bool operator!=(const vec3i& a, const vec3i& b) { return not operator==(a,b); }

// 3d lexical ordering  ------------------------------
inline bool operator<(const vec3i& a, const vec3i& b) { return a.x<b.x or (a.x==b.x and a.y<b.y) or (a.x==b.x and a.y==b.y and a.z<b.z); }
...
inline bool operator!=(const vec4i& a, const vec4i& b) { return not operator==(a,b); }

// 4d lexical ordering  ------------------------------
inline bool operator<(const vec4i& a, const vec4i& b) { return a.x<b.x or (a.x==b.x and a.y<b.y) or (a.x==b.x and a.y==b.y and a.z<b.z) or (a.x==b.x and a.y==b.y and a.z==b.z and a.w<b.w); }
...
inline bool operator!=(const frame3f& a, const frame3f& b) { return not operator==(a,b); }
...
struct frame2f {
    ...
    // default constructor (identity frame)
    frame2f() : o(0, 0), x(1, 0), y(0, 1) { }
    // element-setting constructor
    frame2f(const vec2f& o, const vec2f& x, const vec2f& y) : o(o), x(x), y(y) { }
    // Vector constructor
    template<typename T> frame2f(const vector<T>& v) { error_if_not(v.size() == size, "bad length"); std::copy(v.begin(),v.end(),begin()); }
...
// component-wise equality --------------------------
inline bool operator==(const frame2f& a, const frame2f& b) { return a.o==b.o and a.x==b.x and a.y==b.y; }
inline bool operator!=(const frame2f& a, const frame2f& b) { return not operator==(a,b); }

// frames vector operations -------------------------
// inverse of a frame
inline frame2f inverse(const frame2f& f) { return frame2f(vec2f(-dot(f.o,f.x), -dot(f.o,f.y)), vec2f(f.x.x, f.y.x),vec2f(f.x.y, f.y.y)); }

// orthogonal
inline vec2f orthogonal(const vec2f& x) { return {x.y,-x.x}; }

// orthogonalize (zxy order)
inline frame2f orthonormalize(const frame2f& f) { auto ret = f; ret.x = normalize(f.x); ret.y = orthogonal(ret.x); return ret; }
// creates a frame from z
inline frame2f frame_from_x(const vec2f& o, const vec2f& x) { auto f = identity_frame2f; f.o = o; f.x = normalize(x); f.y = orthogonal(f.x); return f; }

// frames creation ----------------------------------
// frame at eye that looks at center (flipped changes z direction)
inline frame2f lookat_frame(const vec2f& eye, const vec2f& center, bool flipped = false) { auto f = frame2f(); f.o = eye; f.x = normalize(center-eye); if(flipped) f.x = -f.x; f.y = orthogonal(f.x); return f; }

// frame-element transforms -------------------------
// transform a point by a frame
inline vec2f transform_point(const frame2f& f, const vec2f& v) { return f.o + f.x * v.x + f.y * v.y; }
// transform a vector by a frame
inline vec2f transform_vector(const frame2f& f, const vec2f& v) { return f.x * v.x + f.y * v.y; }
// transform a vector by a direciton
inline vec2f transform_direction(const frame2f& f, const vec2f& v) { return transform_vector(f,v); }
// transform a normal by a frame
inline vec2f transform_normal(const frame2f& f, const vec2f& v) { return transform_vector(f,v); }
// transform a frame by a frame
inline frame2f transform_frame(const frame2f& f, const frame2f& v) { return frame2f(transform_point(f,v.o), transform_vector(f,v.x), transform_vector(f,v.y)); }

// frame-element inverse transforms ------------------
// transform a point by a frame inverse
inline vec2f transform_point_inverse(const frame2f& f, const vec2f& v) { return vec2f(dot(v-f.o,f.x), dot(v-f.o,f.y)); }
// transform a vector by a frame inverse
inline vec2f transform_vector_inverse(const frame2f& f, const vec2f& v) { return vec2f(dot(v,f.x), dot(v,f.y)); }
// transform a direction by a frame inverse
inline vec2f transform_direction_inverse(const frame2f& f, const vec2f& v) { return transform_vector_inverse(f,v); }
// transform a normal by a frame inverse
inline vec2f transform_normal_inverse(const frame2f& f, const vec2f& v) { return transform_vector_inverse(f,v); }
// transform a frame by a frame inverse
inline frame2f transform_frame_inverse(const frame2f& f, const frame2f& v) { return frame2f(transform_point_inverse(f,v.o), transform_vector_inverse(f,v.x), transform_vector_inverse(f,v.y)); }
...
struct frame2d {
    ...
    // default constructor (identity frame)
    frame2d() : o(0, 0), x(1, 0), y(0, 1) { }
    // element-setting constructor
    frame2d(const vec2d& o, const vec2d& x, const vec2d& y) : o(o), x(x), y(y) { }
    // Vector constructor
    template<typename T> frame2d(const vector<T>& v) { error_if_not(v.size() == size, "bad length"); std::copy(v.begin(),v.end(),begin()); }
    ...
    typedef double value_type;
    static const int size = 6;
    double* data() { return &o.x; }
    const double* data() const { return &o.x; }
};
...
// component-wise equality --------------------------
inline bool operator==(const frame2d& a, const frame2d& b) { return a.o==b.o and a.x==b.x and a.y==b.y; }
inline bool operator!=(const frame2d& a, const frame2d& b) { return not operator==(a,b); }

// frames vector operations -------------------------
// inverse of a frame
inline frame2d inverse(const frame2d& f) { return frame2d(vec2d(-dot(f.o,f.x), -dot(f.o,f.y)), vec2d(f.x.x, f.y.x),vec2d(f.x.y, f.y.y)); }

// orthogonal
inline vec2d orthogonal(const vec2d& x) { return {x.y,-x.x}; }

// orthogonalize (zxy order)
inline frame2d orthonormalize(const frame2d& f) { auto ret = f; ret.x = normalize(f.x); ret.y = orthogonal(ret.x); return ret; }
// creates a frame from z
inline frame2d frame_from_x(const vec2d& o, const vec2d& x) { auto f = identity_frame2d; f.o = o; f.x = normalize(x); f.y = orthogonal(f.x); return f; }

// frames creation ----------------------------------
// frame at eye that looks at center (flipped changes z direction)
inline frame2d lookat_frame(const vec2d& eye, const vec2d& center, bool flipped = false) { auto f = frame2d(); f.o = eye; f.x = normalize(center-eye); if(flipped) f.x = -f.x; f.y = orthogonal(f.x); return f; }

// frame-element transforms -------------------------
// transform a point by a frame
inline vec2d transform_point(const frame2d& f, const vec2d& v) { return f.o + f.x * v.x + f.y * v.y; }
// transform a vector by a frame
inline vec2d transform_vector(const frame2d& f, const vec2d& v) { return f.x * v.x + f.y * v.y; }
// transform a vector by a direciton
inline vec2d transform_direction(const frame2d& f, const vec2d& v) { return transform_vector(f,v); }
// transform a normal by a frame
inline vec2d transform_normal(const frame2d& f, const vec2d& v) { return transform_vector(f,v); }
// transform a frame by a frame
inline frame2d transform_frame(const frame2d& f, const frame2d& v) { return frame2d(transform_point(f,v.o), transform_vector(f,v.x), transform_vector(f,v.y)); }

// frame-element inverse transforms ------------------
// transform a point by a frame inverse
inline vec2d transform_point_inverse(const frame2d& f, const vec2d& v) { return vec2d(dot(v-f.o,f.x), dot(v-f.o,f.y)); }
// transform a vector by a frame inverse
inline vec2d transform_vector_inverse(const frame2d& f, const vec2d& v) { return vec2d(dot(v,f.x), dot(v,f.y)); }
// transform a direction by a frame inverse
inline vec2d transform_direction_inverse(const frame2d& f, const vec2d& v) { return transform_vector_inverse(f,v); }
// transform a normal by a frame inverse
inline vec2d transform_normal_inverse(const frame2d& f, const vec2d& v) { return transform_vector_inverse(f,v); }
// transform a frame by a frame inverse
inline frame2d transform_frame_inverse(const frame2d& f, const frame2d& v) { return frame2d(transform_point_inverse(f,v.o), transform_vector_inverse(f,v.x), transform_vector_inverse(f,v.y)); }
...
inline bool operator!=(const mat4f& a, const mat4f& b) { return not operator==(a,b); }
...
inline bool operator!=(const range1f& a, const range1f& b) { return not operator==(a,b); }
...
// 3d component-wise equality  -----------------------
inline bool operator==(const range2f& a, const range2f& b) { return a.min==b.min and a.max==b.max; }
inline bool operator!=(const range2f& a, const range2f& b) { return not operator==(a,b); }

// 3d bounding interval operatons  -------------------
inline bool isvalid(const range2f& a) { return min(a.min,a.max) == a.min; }
inline bool contains(const range2f& a, const vec2f& b) { return b.x >= a.min.x and b.x <= a.max.x and b.y >= a.min.y and b.y <= a.max.y; }
inline bool overlaps(const range2f& a, const range2f& b) { return isvalid({max(a.min, b.min),min(a.max,b.max)}); }
inline vec2f size(const range2f& a) { return (a.max-a.min); }
inline vec2f center(const range2f& a) { return (a.max+a.min)/2; }
inline range2f runion(const range2f& a, const vec2f& b) { if(not isvalid(a)) return range2f(b,b); return range2f(min(a.min,b),max(a.max,b)); }
inline range2f runion(const range2f& a, const range2f& b) { if(not isvalid(a)) return b; if(not isvalid(b)) return a; return range2f(min(a.min,b.min),max(a.max,b.max)); }
inline range2f rscale(const range2f& a, float b) { return range2f(center(a)-size(a)*b/2,center(a)+size(a)*b/2); }
inline range2f make_range2f(const std::initializer_list<vec2f>& points) { auto bbox = range2f(); for(auto& p : points) bbox = runion(bbox,p); return bbox; }
inline range2f make_range2f(const vector<vec2f>& points) { auto bbox = range2f(); for(auto& p : points) bbox = runion(bbox,p); return bbox; }

// 2D Bounding Box
struct range2d {
    vec2d min; // min corner
    vec2d max; // max corner
    
    // Default constructor (set to invalid)
    range2d() : min(+1,+1), max(-1,-1) { }
    // Element-wise constructor
    range2d(const vec2d& min, const vec2d& max) : min(min), max(max) { }
    
    // type system
    typedef double value_type;
    static const int size = 4;
    double* data() { return &min.x; }
    const double* data() const { return &min.x; }
};

// 3d component-wise equality  -----------------------
inline bool operator==(const range2d& a, const range2d& b) { return a.min==b.min and a.max==b.max; }
inline bool operator!=(const range2d& a, const range2d& b) { return not operator==(a,b); }

// 3d bounding interval operatons  -------------------
inline bool isvalid(const range2d& a) { return min(a.min,a.max) == a.min; }
inline bool contains(const range2d& a, const vec2d& b) { return b.x >= a.min.x and b.x <= a.max.x and b.y >= a.min.y and b.y <= a.max.y; }
inline bool overlaps(const range2d& a, const range2d& b) { return isvalid({max(a.min, b.min),min(a.max,b.max)}); }
inline vec2d size(const range2d& a) { return (a.max-a.min); }
inline vec2d center(const range2d& a) { return (a.max+a.min)/2; }
inline range2d runion(const range2d& a, const vec2d& b) { if(not isvalid(a)) return range2d(b,b); return range2d(min(a.min,b),max(a.max,b)); }
inline range2d runion(const range2d& a, const range2d& b) { if(not isvalid(a)) return b; if(not isvalid(b)) return a; return range2d(min(a.min,b.min),max(a.max,b.max)); }
inline range2d rscale(const range2d& a, double b) { return range2d(center(a)-size(a)*b/2,center(a)+size(a)*b/2); }
inline range2d make_range2d(const std::initializer_list<vec2d>& points) { auto bbox = range2d(); for(auto& p : points) bbox = runion(bbox,p); return bbox; }
inline range2d make_range2d(const vector<vec2d>& points) { auto bbox = range2d(); for(auto& p : points) bbox = runion(bbox,p); return bbox; }
...
inline bool operator!=(const range3f& a, const range3f& b) { return not operator==(a,b); }
...
inline range3f make_range3f(const vector<vec3f>& points) { auto bbox = range3f(); for(auto& p : points) bbox = runion(bbox,p); return bbox; }
inline array<vec3f,8> corners(const range3f& a) { array<vec3f,8> ret; ret[0] = vec3f(a.min.x,a.min.y,a.min.z); ret[1] = vec3f(a.min.x,a.min.y,a.max.z); ret[2] = vec3f(a.min.x,a.max.y,a.min.z); ret[3] = vec3f(a.min.x,a.max.y,a.max.z); ret[4] = vec3f(a.max.x,a.min.y,a.min.z); ret[5] = vec3f(a.max.x,a.min.y,a.max.z); ret[6] = vec3f(a.max.x,a.max.y,a.min.z); ret[7] = vec3f(a.max.x,a.max.y,a.max.z); return ret; }
...
// 1d component-wise equality  -----------------------
inline bool operator==(const range1i& a, const range1i& b) { return a.min==b.min and a.max==b.max; }
inline bool operator!=(const range1i& a, const range1i& b) { return not operator==(a,b); }
...
inline range1i runion(const range1i& a, int b) { if(not isvalid(a)) return range1i(b,b); return range1i(min(a.min,b),max(a.max,b)); }
inline range1i range2fion(const range1i& a, const range1i& b) { if(not isvalid(a)) return b; if(not isvalid(b)) return a; return range1i(min(a.min,b.min),max(a.max,b.max)); }
...
// 3d component-wise equality  -----------------------
inline bool operator==(const range2i& a, const range2i& b) { return a.min==b.min and a.max==b.max; }
inline bool operator!=(const range2i& a, const range2i& b) { return not operator==(a,b); }
...
inline range2i runion(const range2i& a, const vec2i& b) { if(not isvalid(a)) return range2i(b,b); return range2i(min(a.min,b),max(a.max,b)); }
inline range2i runion(const range2i& a, const range2i& b) { if(not isvalid(a)) return b; if(not isvalid(b)) return a; return range2i(min(a.min,b.min),max(a.max,b.max)); }
...
// 3d component-wise equality  -----------------------
inline bool operator==(const range3i& a, const range3i& b) { return a.min==b.min and a.max==b.max; }
inline bool operator!=(const range3i& a, const range3i& b) { return not operator==(a,b); }
...
inline range3i runion(const range3i& a, const vec3i& b) { if(not isvalid(a)) return range3i(b,b); return range3i(min(a.min,b),max(a.max,b)); }
inline range3i runion(const range3i& a, const range3i& b) { if(not isvalid(a)) return b; if(not isvalid(b)) return a; return range3i(min(a.min,b.min),max(a.max,b.max)); }
